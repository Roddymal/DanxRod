...
public class VehicleMovement : MonoBehaviour
{
	...
	public float slowingVelFactor = .999f;   //The percentage of velocity the ship maintains when not thrusting (e.g., a value of .99 means the ship loses 1% velocity when not thrusting)
	public float brakingVelFactor = .98f;   //The percentage of velocty the ship maintains when braking
	public float angleOfRoll = 30f;			//The angle that the ship "banks" into a turn
    public float DriftIntensity = 2;        // How much the car throws its back out
    ...
    public LayerMask whatIsAlmostGround;    //A layer mask to determine what layers act almost like ground
    ...
    public float zoomForce = 100f;           // Force applied when Zooming
    ...
    public bool isOnAlmostGround;               //A flag determining if the ship is currently on almost ground
    ...
    void CalculatHover()
    {
        ...
        isOnAlmostGround = Physics.Raycast(ray, out hitInfo, maxGroundDist, whatIsAlmostGround);
        ...
        //...Otherwise...
        //If the ship is on almost ground...
        else if (isOnAlmostGround)
        {
        ...
        //Vector3 jumpProjection = Vector3.Project(transform.up, groundNormal);
        ...
        if (input.isJumping)
        {
            if (isOnGround || isOnAlmostGround)
            {
                rigidBody.AddForce(groundNormal * jumpForce, ForceMode.Acceleration);
                rigidBody.AddForce(projection * zoomForce, ForceMode.Acceleration);
            }
	...
	void CalculatePropulsion()
	{
		...
		Vector3 sideFriction = -transform.right * (sidewaysSpeed / (Time.fixedDeltaTime)) / DriftIntensity; 
